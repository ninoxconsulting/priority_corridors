...
#srast <- rast(file.path("01_inputs", "sk_rast_template.tif"))
#srast1km <- aggregate(srast, fact = 10)

in_aoi <- st_read(file.path("01_inputs", "bc_buff_aoi.gpkg"))

in_aoi <- st_as_sf(in_aoi)
out_dir <- fs::path("01_inputs")
...
st_write(lakes1, file.path(out_dir, "lakes.gpkg"), append = FALSE)


# rivers 

ri <- bcdc_query_geodata("f7dac054-efbf-402f-ab62-6fc4b32a619e") |>
  filter(INTERSECTS(in_aoi)) |>
  select( WATERBODY_TYPE, AREA_HA) |>
  collect()
...
st_write(ri, file.path(out_dir, "rivers.gpkg"), append = FALSE)
...
st_write(wetlands, file.path(out_dir, "wetlands.gpkg"), append = FALSE)


# # determine percent density per pixel (100m x 100m) 
# wl <- st_read(file.path("01_inputs", "wetlands.gpkg"))
# 
...
st_write(pro, file.path("01_inputs", "protected_lands.gpkg"), append = FALSE)
...
st_write(cons, file.path("01_inputs", "cons_lands.gpkg"), append = FALSE)
...
st_write(pcon, file.path("01_inputs", "bc_conservancy.gpkg"), append = FALSE)
...
st_write(ec , file.path("01_inputs", "bc_ecoreg.gpkg"), append = FALSE)
...
list.files(file.path('C:/Users/genev/OneDrive/Documents/02.Contracts/00_data/base_vector/bc/BC_TAP_Forestry_data/')
iw <- st_read(file.path('C:/Users/genev/OneDrive/Documents/02.Contracts/00_data/base_vector/bc/BC_TAP_Forestry_data/BCVW_tap_watewrshed_data/OGSR_TAP_INTACT_WATERSHEDS_SP/',"OGSR_TIW_polygon.shp"))

st_write(iw , file.path("01_inputs", "TAP_intactws.gpkg"), append = FALSE)
...
ref_path <- file.path('C:/Users/genev/OneDrive/Documents/02.Contracts/00_data/base_vector/canada', "Stolar_et_al_2024_CiCP_Zenodo_upload_Version_1.1")
...
st_write(cri, file.path("01_inputs", "fed_listed_sp_raw.gpkg"))
...
est <- file.path('C:/Users/genev/OneDrive/Documents/02.Contracts/00_data/base_vector/global', "Pacific estuary ranking shape files")

est <- st_read(file.path(est, "PECP_estuary_polys_ranked_2019_PUBLIC.shp"))

est <- st_intersection(est, in_aoi)

st_write(est, file.path("01_inputs", "Pacific_estuary_rank.gpkg"))
...
st_write(eco, file.path("01_inputs", "bc_red_blue_sp_raw.gpkg"))
...
#download and crop to stohlers boundaru 
template <- rast(file.path("01_inputs", "microrefugia.tif"))


lfiles <- file.path('C:/Users/genev/OneDrive/Documents/02.Contracts/00_data/base_vector/canada/Pither_etal')

con <- rast(file.path(lfiles , "Current_Density_1_Conductance.tif"))
res <- rast(file.path(lfiles , "Current_Density_2_Resistance.tif"))
cost <- rast(file.path(lfiles , "Movement_Cost_Layer.tif"))
...
writeRaster(conn , file.path("01_inputs", "pither_conductance.tif"), overwrite = TRUE)
...
writeRaster(ress , file.path("01_inputs", "pither_resistance.tif"), overwrite = TRUE)
...
writeRaster(costt , file.path("01_inputs", "pither_move_cost.tif"), overwrite = TRUE)
